...
static int open_and_lock(const char* file) {
  int fd;
 ...
 */
static void emergency_invalidate_counter(int fd) {
  int r;
  ...
  l = (count >= 0) ? intlen(count) : 1;
  {
    ...
    for (;;) {
      ...
      if (r < 0) {
        ...
        if (w > 0) emergency_invalidate_counter(fd);
        return -1;
      }
    ...
    if (r < 0) emergency_invalidate_counter(fd);
  }
  return r;
}
...
PAM_EXTERN int pam_sm_close_session(pam_handle_t* pamh, int flags, int argc,
  ...
  if (r != PAM_SUCCESS) return (r == PAM_NO_MODULE_DATA) ? PAM_SUCCESS : r;

  if (geteuid() != 0) return PAM_SESSION_ERR;

  if (!ensure_parent_dir()) return PAM_SESSION_ERR;

  r = pam_get_user(pamh, &user, NULL);
  if (r != PAM_SUCCESS) return PAM_USER_UNKNOWN;
...
struct pam_module _pam_rundir_modstruct = {
    "pam_rundir",         NULL, NULL, NULL, pam_sm_open_session,
